<template>
	<nav>
		<v-app-bar app color="primary" dark clipped-left>
			...
			<v-tooltip bottom>
				<template v-slot:activator="{ on }">
					<v-btn text v-on="on" to="/stream" replace>
						<v-icon color="#ff0000">mdi-circle</v-icon>
						<span class="ml-3">Go Live</span>
					</v-btn>
				</template>
				<span>Go Live</span>
			</v-tooltip>
			...
			<v-menu offset-y left transition="slide-y-transition">
				...
				<v-list>
					<v-list-item @click.stop="$vuetify.theme.dark = !$vuetify.theme.dark">
						...
						>
						<v-btn text :class="$vuetify.theme.dark ? 'd-flex' : 'd-none'"
							>Enable Light Theme</v-btn
						>
						<v-btn text :class="$vuetify.theme.dark ? 'd-none' : 'd-flex'"
							>Enable Dark Theme</v-btn
						>
					</v-list-item>
		...
		>
			...
			<v-row justify="space-around">
				<span class="white--text subtitle mt-2">Student</span>
			</v-row>
			...
			<v-list dense>
				...
				<v-list-item to="/profile" replace>
					<v-list-item-icon>
						<v-icon>mdi-account-circle</v-icon>
					</v-list-item-icon>
					<span class="subtitle-1 font-weight-medium">Profile</span>
				</v-list-item>
